...
template< table_names TableName >
score evaluate(const chess::game_board &board)
{
    ...
    for(unsigned int i = chess::w_pawn; i < chess::piece_count; i++)
    {
        ...
        score += eval_table<TableName>::material[i] * bit_count(bb);
    }
    ...
    for(unsigned int i = chess::w_pawn; i < chess::piece_count; i++)
    {
        ...
        while(bb)
        {
            ...
            score += (color == chess::white) ? eval_table<TableName>::position[i % 6][sq] : -eval_table<TableName>::position[i % 6][sq];
        }
    }
